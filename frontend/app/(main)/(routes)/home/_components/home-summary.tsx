  ...
  const chartData = useMemo(() => {
    ...
    if (period === "today") {
      ...
      return hours.map((hour, index) => {
        const currentHourTxs = transactions.filter(tx => {
          const txDate = parseISO(tx.newDate || tx.originalDate);
          return isSameHour(txDate, hour) && tx.type === 'debit' && 
                 tx.categoryId?.name !== 'Investment' && tx.categoryId?.name !== 'Self Transfer';
        });
        
        const prevHourTxs = transactions.filter(tx => {
          const txDate = parseISO(tx.newDate || tx.originalDate);
          return isSameHour(txDate, prevHours[index]) && tx.type === 'debit' && 
                 tx.categoryId?.name !== 'Investment' && tx.categoryId?.name !== 'Self Transfer';
        });
    ...
    if (period === "week") {
      ...
      return days.map((day, index) => {
        const currentDayTxs = transactions.filter(tx => {
          const txDate = parseISO(tx.newDate || tx.originalDate);
          return isSameDay(txDate, day) && tx.type === 'debit' && 
                 tx.categoryId?.name !== 'Investment' && tx.categoryId?.name !== 'Self Transfer';
        });
        
        const prevDayTxs = transactions.filter(tx => {
          const txDate = parseISO(tx.newDate || tx.originalDate);
          return isSameDay(txDate, prevDays[index]) && tx.type === 'debit' && 
                 tx.categoryId?.name !== 'Investment' && tx.categoryId?.name !== 'Self Transfer';
        });
    ...
    return days.map((day, index) => {
      ...
      if (prevDay) {
        const prevDayTxs = transactions.filter(tx => {
          const txDate = parseISO(tx.newDate || tx.originalDate);
          return isSameDay(txDate, prevDay) && tx.type === 'debit' && 
                 tx.categoryId?.name !== 'Investment' && tx.categoryId?.name !== 'Self Transfer';
        });
      ...
      <div className="bg-card dark:bg-[#111111] border border-border dark:border-white/5 rounded-[2.5rem] p-8 space-y-8 relative overflow-hidden group shadow-sm">
        <div className="space-y-6">
          <button 
            ...
            className="flex items-center gap-2 text-muted-foreground font-bold hover:text-primary transition-colors group/btn"
          >
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 pt-2">
                <div className={cn(
                  ...
                  isDown ? "text-emerald-500 bg-emerald-500/10" : "text-rose-500 bg-rose-500/10"
                )}>
                  {isDown ? <ArrowDown className="h-3 w-3" /> : <ArrowUp className="h-3 w-3" />}
                  ₹{formatAmount(Math.abs(diff))}
                </div>
                <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">
                  vs {period === "today" ? "yesterday" : period === "week" ? "last week" : "last month"}
                </span>
              </div>
            </div>
