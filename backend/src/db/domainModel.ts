...
const DomainSchema = new mongoose.Schema({
  ...
  subject: { type: String },
  parserConfigId: { type: mongoose.Schema.Types.ObjectId, ref: 'ParserConfig' },
}, { timestamps: true });

export const DomainModel = mongoose.model('Domain', DomainSchema);

export const getDomainsByAccountId = (accountId: string) => DomainModel.find({ accountId }).populate('parserConfigId');
export const getDomainById = (id: string) => DomainModel.findById(id).populate('parserConfigId');
